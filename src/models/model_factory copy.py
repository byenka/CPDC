...
def _unwrap_state_dict(state):
    """Ambil state_dict sebenarnya dari berbagai format checkpoint umum."""
...
def _load_flexible(model: nn.Module, state_dict: Dict[str, torch.Tensor], logger: Optional[logging.Logger] = None) -> Tuple[list, list, list]:
    ...
    if logger:
        ...
        if dropped:
            logger.info(f"Dropped {len(dropped)} key(s) due to shape mismatch, e.g.: {', '.join(dropped[:6])}{' ...' if len(dropped)>6 else ''}")
        if missing:
            logger.info(f"Missing keys after load (OK/expected for new head): {len(missing)}")
        if unexpected:
