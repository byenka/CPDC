...
class TrainingMonitor:
    ...
    def update_batch(self, predictions: torch.Tensor, labels: torch.Tensor,
                     loss: float, gradient_norm: float = None, confidences: torch.Tensor = None):
            ...
            for batch_idx in range(num_batches):
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        self.train_monitor.update_batch(preds, labels, loss.item() * accum_steps, None, confs)
    ...
    def validate(self, val_loader: DataLoader, current_epoch: int = 0) -> Dict:
        ...
        with torch.no_grad():
            for images, labels, metadata in val_loader:
                ...
                confs, preds = probs.max(1)
                self.val_monitor.update_batch(preds, labels, float(loss.item()), None, confs)
