...
class BiTemperedLoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        probabilities = self._exp_t(x, self.t2)
        probabilities = probabilities / probabilities.sum(dim=1, keepdim=True).clamp_min(1e-12)

        loss = y * (self._log_t(y.clamp_min(1e-12), self.t1) - self._log_t(probabilities.clamp_min(1e-12), self.t1))
        loss = loss.sum(dim=1)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.logger:
                ...
                self.logger.info(f"  Val   Loss: {val_loss:.4f} | BA: {val_ba:.4f} | F1: {val_f1:.4f} | ECE: {val_ece:.4f} | H: {val_ent:.4f}")
            else:
...
def train(config_path: str):
    ...
    # Check if balanced (ratio < 1.2)
    is_balanced = (counts.max() / counts.min()) < 1.2
    
    if is_balanced:
        print(f"Dataset detected as balanced (ratio {counts.max()/counts.min():.2f}). Using standard shuffle=True.")
        sampler = None
        shuffle_mode = True
    else:
    ...
    enabled = bool(lr_lin.get('enabled', True))
