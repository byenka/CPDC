...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        return epoch_loss, epoch_ba, epoch_f1, epoch_ece, epoch_ent, epoch_nll, epoch_brier, n_samples
    ...
    def validate(self):
        ...
        n_samples = int(labels_all.numel())
        return val_loss, val_ba, val_f1, val_ece, val_ent, val_nll, val_brier, n_samples
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            tr_loss, tr_ba, tr_f1, tr_ece, tr_ent, tr_nll, tr_brier, tr_n = self.train_epoch(epoch)
            val_loss, val_ba, val_f1, val_ece, val_ent, val_nll, val_brier, val_n = self.validate()

            # ringkasan log
            if self.logger:
                self.logger.info(f"\nEpoch {epoch+1}/{self.num_epochs}")
                self.logger.info(
                    f"  Train n={tr_n} | Loss: {tr_loss:.4f} | BA: {tr_ba:.4f} | F1: {tr_f1:.4f} | "
                ...
                print(f"Epoch {epoch+1}/{self.num_epochs} | "
                      f"Train: n={tr_n}, loss={tr_loss:.4f}, BA={tr_ba:.4f}, F1={tr_f1:.4f}, ECE_before={tr_ece:.4f}, NLL_before={tr_nll:.4f}, Brier_before={tr_brier:.4f}, H={tr_ent:.4f} | "
                      f"Val: n={val_n}, loss={val_loss:.4f}, BA={val_ba:.4f}, F1={val_f1:.4f}, ECE_before={val_ece:.4f}, NLL_before={val_nll:.4f}, Brier_before={val_brier:.4f}, H={val_ent:.4f}")
