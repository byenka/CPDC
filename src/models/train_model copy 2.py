...
from src.metrics.metrics import metric_ece, metric_brier_multiclass
...
def calibrate_temperature(model: nn.Module, val_loader: DataLoader, device: torch.device, logger: Optional[logging.Logger]):
        """Cari T optimal yang meminimalkan NLL; laporkan metrik trustworthiness sebelum/sesudah.

        Konsistensi definisi:
            - Before: softmax(logits)
            - After : softmax(logits / T_optimal)
        """
    if logger:
        logger.info("\n" + "="*80)
    ...
    with torch.no_grad():
        ...
        # NLL / Brier computed from probabilities
        probs_before = torch.softmax(logits_all, dim=1).detach().cpu().numpy()
        probs_after  = torch.softmax(logits_all / max(T_opt, 1e-6), dim=1).detach().cpu().numpy()
        y_true = labels_all.detach().cpu().numpy().astype(int)

        nll_before = _nll_from_probs(probs_before, y_true)
        nll_after  = _nll_from_probs(probs_after, y_true)
        brier_before = float(metric_brier_multiclass(probs_before, y_true))
        brier_after  = float(metric_brier_multiclass(probs_after, y_true))
...
def _probs_from_logits(logits: torch.Tensor, temperature: float = 1.0) -> torch.Tensor:
    logits = logits.to(dtype=torch.float32)
    t = float(temperature)
...
def _nll_from_probs(probs: np.ndarray, y_true: np.ndarray) -> float:
    # Mean negative log-likelihood based on probabilities.
...
def _calibration_metrics_from_logits(
    logits: torch.Tensor,
    labels: torch.Tensor,
    temperature: float = 1.0,
    n_bins_ece: int = 15,
) -> Dict[str, float]:
    """Return ECE/NLL/Brier computed from logits with a given temperature."""
    probs_t = _probs_from_logits(logits, temperature=temperature).detach().cpu().numpy()
    y_true = labels.detach().cpu().numpy().astype(int)
    return {
        'ece': float(metric_ece(probs_t, y_true, n_bins=n_bins_ece)),
        'nll': float(_nll_from_probs(probs_t, y_true)),
        'brier': float(metric_brier_multiclass(probs_t, y_true)),
    }
...
class BiTemperedLoss(nn.Module):
    ...
    Catatan:
    - Implementasi sebelumnya adalah bentuk ringkas yang bisa menghasilkan loss negatif
      pada label smoothing tertentu.
    - Implementasi ini mengikuti bentuk bi-tempered logistic loss dengan tempered softmax
      dan generalized KL-divergence sehingga loss  non-negatif (hanya bisa negatif
      kecil karena floating error  kita clamp).
    """
    ...
    def _log_t(self, x, t: float):
        if abs(t - 1.0) < 1e-7:
            return torch.log(x)
        return (x.pow(1 - t) - 1) / (1 - t)

    def _exp_t(self, x, t: float):
        if abs(t - 1.0) < 1e-7:
            return torch.exp(x)
        return torch.relu(1 + (1 - t) * x).pow(1 / (1 - t))

    def _tempered_softmax(self, logits: torch.Tensor, t: float, iters: int = 5) -> torch.Tensor:
        """Tempered softmax with fixed-point normalization for t>1."""
        if abs(t - 1.0) < 1e-7:
        ...
        if t < 1.0:
            # For t<1 the distribution has compact support.
            # A simple normalized exp_t is sufficient for our use-case.
            p = self._exp_t(logits, t)
            return p / p.sum(dim=1, keepdim=True).clamp_min(1e-12)

        # t > 1.0: need normalization constant. Use iterative scaling.
        p = self._exp_t(logits, t)
        for _ in range(max(1, int(iters))):
            p_sum = p.sum(dim=1, keepdim=True).clamp_min(1e-12)
            p = p / p_sum
            # Recompute p with updated normalization (fixed-point)
            log_z = p_sum.pow(1 - t)
            p = self._exp_t(logits - self._log_t(log_z, t), t)

        return p / p.sum(dim=1, keepdim=True).clamp_min(1e-12)

    def _generalized_kl(self, y: torch.Tensor, p: torch.Tensor, t: float) -> torch.Tensor:
        """Generalized KL divergence for temperature t (t1 in paper)."""
        y = y.clamp_min(1e-12)
        p = p.clamp_min(1e-12)

        if abs(t - 1.0) < 1e-7:
            # Standard KL: sum y * log(y/p)
            return (y * (torch.log(y) - torch.log(p))).sum(dim=1)

        # Generalized KL used by bi-tempered logistic loss.
        term1 = y * (self._log_t(y, t) - self._log_t(p, t))
        term2 = (y.pow(2 - t) - p.pow(2 - t)) / (2 - t)
    ...
    def forward(self, logits, target):
        ...
        probs = self._tempered_softmax(logits, t=self.t2)
        loss = self._generalized_kl(y, probs, t=self.t1)

        # Numerical safety: loss should be non-negative; clamp tiny negatives.
...
class Trainer:
    ...
    def __init__(
        ...
        # Default defensible choice for DR multiclass: Balanced Accuracy on val_v2.
        self.save_by = str(ckpt_cfg.get('save_by', 'balanced_accuracy')).lower()
    ...
    def validate(self):
        ...
        # Trustworthiness metrics (Before = raw softmax(logits))
        cal_before = _calibration_metrics_from_logits(logits_all, labels_all, temperature=1.0, n_bins_ece=15)
        val_n_samples = int(labels_all.numel())

        return val_loss, val_ba, val_f1, val_ece, val_ent, val_n_samples, cal_before
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            val_loss, val_ba, val_f1, val_ece, val_ent, val_n, cal_before = self.validate()

            # ringkasan log
            if self.logger:
                self.logger.info(f"\nEpoch {epoch+1}/{self.num_epochs}")
                self.logger.info(f"  Train Loss: {tr_loss:.4f} | BA: {tr_ba:.4f} | F1: {tr_f1:.4f} | ECE: {tr_ece:.4f} | H: {tr_ent:.4f}")
                self.logger.info(
                    f"  Val   n={val_n} | Loss: {val_loss:.4f} | BA: {val_ba:.4f} | F1: {val_f1:.4f} | "
                    f"ECE_before: {cal_before['ece']:.4f} | NLL_before: {cal_before['nll']:.4f} | Brier_before: {cal_before['brier']:.4f}"
                )
            else:
            ...
            # top-k keeper (utama)
            saved_path = self.topk.consider(epoch + 1, monitor_value, self.model.state_dict(), f"{tag}=")
...
def train(config_path: str):
    ...
    if base_config.get('robust_loss'):
    ...
    # Datasets & manifests
    processed_dir = base_config['paths']['processed_dir']
