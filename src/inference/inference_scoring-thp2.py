...
def _resolve_manifest_for_mode(base_config, mode: str) -> str:
    """
    ...
    if mode == 'master':
        return os.path.join(processed, 'master_list.csv')
    elif mode == 'train':
        return manifests.get('train') or os.path.join(processed, 'gold_standard_train.csv')
    elif mode == 'val':
        return manifests.get('validate') or os.path.join(processed, 'gold_standard_validate.csv')
    elif mode == 'gold_standard':
        train_csv = manifests.get('train') or os.path.join(processed, 'gold_standard_train.csv')
        val_csv   = manifests.get('validate') or os.path.join(processed, 'gold_standard_validate.csv')
...
def find_q_threshold_roc(ae_model, base_config, device, logger=None):
    ...
    good_manifest_path = base_config['manifests'].get('validate') or \
                         os.path.join(base_config['paths']['processed_dir'], 'gold_standard_validate.csv')
...
def load_ensemble_a(config_paths, base_config, device, logger=None):
    ...
    # Default configs (convnext, dinov2, clipvit)
...
def _export_tier_manifests(results_df: pd.DataFrame, base_config: dict, manifest_df: pd.DataFrame):
    ...
    # Kolom minimal untuk manifest turunannya
    wanted_cols = []
    ...
    A[wanted_cols].to_csv(tier_a_path, index=False)
    B[wanted_cols].to_csv(tier_b_path, index=False)
    C[wanted_cols].to_csv(tier_c_path, index=False)
...
def run_full_inference(base_config_path='configs/base_config_coweps.yaml',
    ...
    # 2) Load Ensemble A
    ensemble_a, temperatures = load_ensemble_a(ensemble_a_configs, base_config, device, logger)
    ...
    print("="*80)
    manifest_path = _resolve_manifest_for_mode(base_config, mode)
    ...
    with torch.no_grad():
        for batch in tqdm(main_loader, desc="CoWePS Inference"):
            ...
            # (a) Ensemble A → probs (T-scaling per model → average)
            scaled_logits_list = []
            ...
            probs = torch.softmax(avg_scaled_logits, dim=1)

            C_score, Pred_Class = probs.max(dim=1)
            entropy, margin = _probs_entropy_margin(probs)
